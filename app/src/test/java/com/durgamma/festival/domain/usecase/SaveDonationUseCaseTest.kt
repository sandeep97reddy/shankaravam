...
class SaveDonationUseCaseTest {
    ...
    fun valid_cash_donation_is_saved_with_sane_defaults() = runTest {
        ...
        assertTrue(result is com.durgamma.festival.core.util.Outcome.Ok, "expected Ok, got $result")
        val saved = (result as com.durgamma.festival.core.util.Outcome.Ok).value
        assertEquals("Ramesh", saved.donorName)
        assertEquals(1000L, saved.addedTime)
        assertEquals(DonationStatus.RECEIVED, saved.status)
        assertEquals(com.durgamma.festival.domain.model.SyncStatus.PENDING_UPLOAD, saved.syncStatus)
    ...
    fun blank_donor_name_is_rejected_without_side_effects() = runTest {
        ...
        assertTrue(result is com.durgamma.festival.core.util.Outcome.Err)
