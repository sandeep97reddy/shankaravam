...
## 1. Where We Are
- **Access Hardening Batch (uncommitted, 14-09-2026):** P3 `canWriteMoney` gate in all 4 money VMs + `canAddExpense` viewer fix; creator-or-admin gates for close-code/promote/revoke/approve (`canManageTeam/canManageCode/canApproveNow`); server-side invite binding (`viaCode` stamp + `isLiveCode` rules + Timestamp `expiresAt`); `r2-lifecycle.json` codifying the dashboard 30d audio rule. Verify: 175/175 green + assembleDebug + rules dry-run compiles. **Next: `firebase deploy --only firestore:rules` (real deploy), then two-device join/approve/revoke matrix.**
- **R2 Media-Sharing Phase 0–3 (uncommitted, 14-09-2026):** Phase 0 local fixes (T0.1 purge/quarantine, T0.2 latest-wins effective amounts, T0.3 audio-status timestamp drop, T0.4 read-cap/phrase-prune, T0.5 Shubh/Pooja lineup); Phase 2 Worker code (`tools/worker/`: JWKS+seat auth, audio resolve, receipts, quotas — 15 vitest + tsc clean, deploy + live matrix is user's turn per `tools/worker/README.md`); Phase 3 Android (Room v3 receiptUrl + merge ingest, ReceiptUploadWorker + Coil view + gateway URL card, CAS audio dual-read + cloud-first engine + prefetch rewire + 429 pill). Verify: `assembleDebug` + 31 suites green. User setup done: bucket `shankaravam-media` + 30d `audio/` lifecycle + `shankaravam-gateway` + `AUDIO_BUCKET` + `SARVAM_API_KEY`; still needed from user: 2 plain Firebase vars, `wrangler deploy`, `verify.mjs` output, gateway URL in app, two-device pilot. Next: Phase 4 key-removal only after pilot. Full file list in `PROGRESS.md` R2 section.
